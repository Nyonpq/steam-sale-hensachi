...
export default function RankCard({ deal }: { deal: Deal }) {
  ...
  return (
    <a href={deal.storeUrl} target="_blank" rel="noopener noreferrer nofollow" className="group block border-2 border-ink bg-card transition-shadow hover:shadow-[4px_4px_0_0_#211D19] sm:grid sm:grid-cols-[auto_auto_1fr_auto] sm:items-center sm:gap-6 sm:px-6 sm:py-4">

      {/* モバイル専用：画像を上部フル幅に表示し、順位と偏差値を重ねる */}
      ...
      {/* デスクトップ専用：順位 */}
      <div className="hidden h-12 w-12 flex-none items-center justify-center border-2 border-ink font-mono text-lg font-bold text-ink sm:flex">{deal.rank}</div>

      {/* デスクトップ専用：偏差値スタンプ */}
      <div className={`stamp-rotate hidden h-16 w-16 flex-none flex-col items-center justify-center rounded-full border-4 font-display sm:flex ${label.ringClass}`}>
      ...
      {/* ゲーム情報 */}
      <div className="min-w-0 px-4 py-3 sm:px-0 sm:py-0">
      ...
      {/* 価格 */}
      <div className="flex-none border-t-2 border-ink px-4 py-3 text-right sm:border-t-0 sm:px-0 sm:py-0">
        <p className="font-mono text-[11px] text-ink-muted line-through">{formatYen(deal.initialPriceCents)}</p>
        <p className="font-display text-lg font-black text-hanko">{formatYen(deal.finalPriceCents)}</p>
        <p className="font-mono text-[11px] font-bold text-hanko">-{deal.discountPercent}%</p>
      </div>
    </a>
  );
}